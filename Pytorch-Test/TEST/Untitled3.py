...
use_GPU = True
model_label_path = "/media/Share/jingyun/Pytorch-training/files/snapshot/NEWFoodRecongnition-res50-classes.txt"
...
model_name = "resnet50"
model_path = "/media/Share/jingyun/Pytorch-training/files/snapshot/BestWeight-NEWFoodRecongnition-res50-06-09-2019_15-31-47.pth"
...
val_dict_img_list = dataset_list_JJ("/media/Share/jingyun/data/Totest_Alan")
...
for entry in food_results:
    ...
    for entry1 in entry["first 10 results"]:  
        if entry1["name"]==entry["GTs"]:
            confi=entry1["confidence"]
    resStartCol+=1        
    ws.cell(row=resStartRow,column=resStartCol,value="{} ({:0.3f})".format(entry["GTs"], confi))
resStartRow += 1
...
#resStartRow += 1
errAnaWorkbook.save("Alan0913.xlsx")
