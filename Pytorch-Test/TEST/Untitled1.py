...
# processing of foodresult

GTs_dict = {}
for entry in food_results:
    if entry['GTs'] not in GTs_dict:
        GTs_dict[entry['GTs']] = [entry]
    else:
        GTs_dict[entry['GTs']].append(entry)
        

for category in GTs_dict.keys():
    save_name = category
    food_results_category = GTs_dict[category]
    ...
    for entry in food_results_category:
        resStartCol = 5
        resStartRow += 1
