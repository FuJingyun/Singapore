...
device = torch.device("cuda:{}".format(training_config["which_gpu"]) if torch.cuda.is_available() and "which_gpu" in training_config else "cpu")
...
train_dataloader = torch.utils.data.DataLoader(food_data,batch_size=train_batchsz,shuffle=True,num_workers=0)
...
print("Using Adam optimiser. Beginning lr:{:.4f}, dropping lr by {} every 1 epoch(s) with no drop in loss".format(get_lr(optimiser), 0.1))
...
if os.path.isfile("{}/{}-classes.txt".format(snapshot_path, dataset_prefix)):
    ...
    if len(tmpDiff) > 0:
        with open("{}/{}-classes-{}.txt".format(snapshot_path, dataset_prefix, timestamp_prefix), "wt", encoding="utf16") as outfile:
    ...
    for phase in phases:
        ...
        for batch_idx, (imgs, labels) in enumerate(dataloader[phase]):
            ...
            if batch_idx and not batch_idx % logger_batch_sz:
                print("{} Batch:{}, loss:{:.5f}, batch running time: {:.0f}:{:02d}".format(phase, batch_idx, loss.item(), batch_time_elapsed / 60, running_corrects % 60))
                tlogger.scalar_summary('{}-running_loss'.format(phase), loss.item() * input_imgs.size(0), batch_idx+1+epoch*len(dataloader[phase]))
                ...
                if logging_model_params:
                    for tag, value in model.named_parameters():
                        tag = tag.replace('.', '/')
                        tlogger.histo_summary("{}/".format(phase)+tag, value.data.cpu().numpy(), batch_idx+1+epoch*len(dataloader[phase]))
                        if not value.grad is None:
                            tlogger.histo_summary("{}/".format(phase)+tag+'/grad', value.grad.cpu().numpy(), batch_idx+1+epoch*len(dataloader[phase]))
        ...
        if (phase == 'val' or len(phases) == 1) and lowest_loss > epoch_loss:
            ...
            best_model_weights = copy.deepcopy(model.state_dict())
            print("Lowest loss so far:{}, snapshotting weights to BestWeight-{}-{}.pth".format(epoch_loss, dataset_prefix, timestamp_prefix))
            torch.save(best_model_weights, "{}/BestWeight-{}-{}.pth".format(snapshot_path, dataset_prefix, timestamp_prefix))
        if phase == 'val' or len(phases) == 1:
            lr_scheduler.step(epoch_loss)

        # snapshot the model every 2 iterations
        if epoch and not epoch % snapshot_every_n_epoch:
            torch.save(model.state_dict(), "{}/{}-{}/{}-{}-{}.pth".format(snapshot_path, dataset_prefix, timestamp_prefix, dataset_prefix, timestamp_prefix, epoch+1))
        ...
        print("{} Epoch:{}, Loss: {:.4f}, lr: {:.8f}, Total running time: {:.0f}:{:.0f}".format(phase, epoch+1, epoch_loss, get_lr(optimiser), time_elapsed / 60, time_elapsed % 60))
