'''
从一个目录下的n个文件夹中，分别抽取一张图片，放入新生成的子目录
Author ： Fu Jingyun
'''
import os
import random
...
#给定操作目录
path='C:/Users/NExT/Desktop/Sort_NBF_test'
newpath = 'C:/Users/NExT/Desktop/Totest_new'
...
for file in dir: 
     ...
     if temppath[-4]=='.':
         if os.path.isfile(temppath):
              shutil.copy(temppath,newsubpath+'/'+str(number)+temppath[-4:])
              number=number+1
         else:
         ...
         if os.path.isfile(temppath):
              shutil.copy(temppath,newsubpath+'/'+str(number)+temppath[-5:])
              number=number+1
         else:
              while not os.path.isfile(temppath):
                  j+=1
